...
import dk.escteam.keyboardextender.protocol.action.FileExploreRequestAction;
import dk.escteam.keyboardextender.protocol.action.FileExploreResponseAction;
...
import dk.escteam.keyboardextender.protocol.action.ScreenCaptureRequestAction;
...
import dk.escteam.keyboardextender.server.tools.UnicodeToSwingKeyCodeConverter;

public class KeyboardExtenderServerConnection implements Runnable
{
	...
	public KeyboardExtenderServerConnection(KeyboardExtenderServerApp application, KeyboardExtenderConnection connection)
	{
		...
		this.useUnicodeWindowsAltTrick = KeyboardExtenderServerApp.IS_WINDOWS && !this.application.getPreferences().getBoolean("force_disable_unicode_windows_alt_trick", false);
	...
	public void run()
	{
		try
		{
			try
			{
				while (true)
				{
					KeyboardExtenderAction action = this.connection.receiveAction();
	...
	private void action(KeyboardExtenderAction action)
	{
	...
	{
		if (action.password.equals(this.application.getPreferences().get("password", KeyboardExtenderConnection.DEFAULT_PASSWORD)))
		{
	...
	private void sendAction(KeyboardExtenderAction action)
	{
		try
		{
